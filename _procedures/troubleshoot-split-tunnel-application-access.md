---
title: Troubleshoot split-tunnel application access
slug: troubleshoot-split-tunnel-application-access
description: 'Enterprise runbook to troubleshoot split-tunnel application access without skipping evidence, verification, rollback or escalation requirements.'
...
tags:
  ...
  - tunnel
error_codes: []
tldr: 'Confirm scope and authorisation, capture evidence, isolate the failing layer, apply the least disruptive approved remediation for troubleshoot split-tunnel application access, verify the original business task, and escalate with complete logs if recovery is not achieved.'
related_symptoms:
  - split-tunnel-application-access
  - network-or-internet-access-is-unavailable
  - connection-is-slow-or-intermittent
symptom_weights:
  split-tunnel-application-access: 10
...
permalink: /procedures/troubleshoot-split-tunnel-application-access/
layout: article
...
---
## Purpose and scope
Use this runbook for **troubleshoot split-tunnel application access** in a managed enterprise environment. It covers intake, evidence, safe diagnosis, remediation, verification, documentation and escalation. It does not replace organisation-specific security, change, safety, privacy, regulatory or vendor procedures.

## Mandatory Batch B-2 controls
These audit-derived controls are mandatory before more invasive remediation.

### Pre-checks
1. Trace the application route from the VPN client and confirm which route should be selected.
2. Check VPN gateway/client route tables and split-tunnel include/exclude policy.
3. Check firewall/ACL access from the VPN address pool to the application subnet.

### Rollback / undo
- Capture route/split-tunnel/ACL configuration before change. Remove or revert the specific change if access worsens; use the platform's supported rollback safety mechanism for remote routing changes.
