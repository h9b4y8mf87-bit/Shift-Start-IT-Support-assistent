---
title: Troubleshoot Citrix printing
slug: troubleshoot-citrix-printing
description: 'Enterprise runbook to troubleshoot citrix printing without skipping evidence, verification, rollback or escalation requirements.'
...
tags:
  ...
  - troubleshoot
error_codes: []
tldr: 'Confirm scope and authorisation, capture evidence, isolate the failing layer, apply the least disruptive approved remediation for troubleshoot citrix printing, verify the original business task, and escalate with complete logs if recovery is not achieved.'
related_symptoms:
  - citrix-printing
...
permalink: /procedures/troubleshoot-citrix-printing/
layout: article
...
---
## Purpose and scope
Use this runbook for **troubleshoot citrix printing** in a managed enterprise environment. It covers intake, evidence, safe diagnosis, remediation, verification, documentation and escalation. It does not replace organisation-specific security, change, safety, privacy, regulatory or vendor procedures.

## Mandatory Batch B-5 controls
These audit-derived controls are mandatory before more invasive remediation.

### Pre-checks
1. Confirm the client printer works locally and determine whether it is mapped into the Citrix session.
2. Check VDA Print Spooler, Citrix printing policies and Universal Print Driver/driver mapping where used.
3. Check Citrix Workspace/VDA version compatibility and whether the issue affects one session, one host or the whole delivery group.

### Rollback / undo
- Capture Citrix printing policy/master-image driver state before change. Restore the previous approved driver/image/policy if a confirmed update causes regression.
...
### Targeted checks
- Check device panel, paper/consumables, queue, driver, port, server, permissions and network reachability.
- Test a device self-page and a simple document from another workstation.

### Targeted remediation sequence
1. Clear only failed jobs, restart the spooler when safe, reconnect the approved driver/port or correct scan destination credentials.
2. Escalate hardware or print-server faults with device logs and test results.
...
4. **Run non-destructive diagnostics.** Check identity, entitlement, gateway reachability, client version, session state, profile, resource capacity and service health.
...
5. **Apply the primary approved remediation.** Reset or reconnect only the affected session, repair the supported client or clear the documented local cache.
...
7. **Apply secondary remediation only when justified.** Move to broker, host, profile-container, capacity or policy investigation when multiple users or repeated sessions are affected.
...
9. **Complete end-to-end verification.** The user launches the assigned desktop or application and can use required audio, printing, storage and display redirection.
