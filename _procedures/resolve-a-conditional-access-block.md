---
title: Resolve a Conditional Access block
slug: resolve-a-conditional-access-block
description: 'Enterprise runbook to resolve a conditional access block without skipping evidence, verification, rollback or escalation requirements.'
...
tags:
  ...
  - resolve
error_codes: []
tldr: 'Confirm scope and authorisation, capture evidence, isolate the failing layer, apply the least disruptive approved remediation for resolve a conditional access block, verify the original business task, and escalate with complete logs if recovery is not achieved.'
related_symptoms:
  - a-conditional-access-block
  - cannot-sign-in
  - access-is-denied-or-missing
symptom_weights:
  a-conditional-access-block: 10
  cannot-sign-in: 3
...
permalink: /procedures/resolve-a-conditional-access-block/
layout: article
...
---
## Purpose and scope
Use this runbook for **resolve a conditional access block** in a managed enterprise environment. It covers intake, evidence, safe diagnosis, remediation, verification, documentation and escalation. It does not replace organisation-specific security, change, safety, privacy, regulatory or vendor procedures.

## Mandatory Batch B-3 controls
These audit-derived controls are mandatory before more invasive remediation.

### Pre-checks
1. Step 0: check Microsoft Entra service health and active tenant incidents before changing Conditional Access.
2. Review sign-in logs and Conditional Access evaluation details to identify the exact blocking policy and reason.
3. Verify device compliance, authentication strength, location/risk and user/group targeting before creating an exception.

### Rollback / undo
- Capture/export the current policy configuration before change. If an authorised policy modification causes wider failure, restore the previous approved configuration or disable only the new change under emergency change control; do not bypass security controls with permanent exclusions.
...
### Targeted checks
- Open the failed sign-in record and identify the exact policy, grant control and device state.
- Check location, risk, platform, client type, compliance and authentication strength.

### Targeted remediation sequence
1. Correct the underlying requirement; never bypass policy by adding an unapproved exclusion.
2. Escalate policy design conflicts with the sign-in correlation ID.

### Scenario-specific success criterion
The same sign-in satisfies the intended policy without an exception.


## Procedure
0. **Check Entra service health before policy changes.** Confirm no active service incident, then use sign-in and Conditional Access evaluation evidence to identify the exact policy and reason for the block.
