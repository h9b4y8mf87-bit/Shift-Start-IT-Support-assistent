---
title: Troubleshoot an electronic-signature workflow
slug: troubleshoot-an-electronic-signature-workflow
description: 'Enterprise runbook to troubleshoot an electronic-signature workflow without skipping evidence, verification, rollback or escalation requirements.'
...
tags:
  ...
  - workflow
error_codes: []
tldr: 'Confirm scope and authorisation, capture evidence, isolate the failing layer, apply the least disruptive approved remediation for troubleshoot an electronic-signature workflow, verify the original business task, and escalate with complete logs if recovery is not achieved.'
related_symptoms:
  - an-electronic-signature-workflow
  - a-business-application-or-website-is-not-working
symptom_weights:
  an-electronic-signature-workflow: 10
...
permalink: /procedures/troubleshoot-an-electronic-signature-workflow/
layout: article
...
---
## Purpose and scope
Use this runbook for **troubleshoot an electronic-signature workflow** in a managed enterprise environment. It covers intake, evidence, safe diagnosis, remediation, verification, documentation and escalation. It does not replace organisation-specific security, change, safety, privacy, regulatory or vendor procedures.

## Mandatory Batch B-4 controls
These audit-derived controls are mandatory before more invasive remediation.

### Pre-checks
1. Check the e-signature provider's official service health before modifying templates/workflows.
2. Verify user entitlement/role, envelope/document state and recipient identity/status.
3. Review the workflow/audit trail for the exact failed step, timestamp and provider error before retrying or voiding.

### Rollback / undo
- Version/capture workflow/template configuration before changes. Restore the previous approved template/workflow if a change caused failure. Void an erroneously sent envelope only under the authorised business/legal process; do not delete its audit trail.
