...
description: Enterprise runbook to resolve low disk space on a server without skipping evidence, verification, rollback
...
tldr: Confirm scope and authorisation, capture evidence, isolate the failing layer, apply the least disruptive approved
  remediation for resolve low disk space on a server, verify the original business task, and escalate with complete
...
escalation: Escalate to Server or Infrastructure Operations with the exact user or service impact, timestamps and
