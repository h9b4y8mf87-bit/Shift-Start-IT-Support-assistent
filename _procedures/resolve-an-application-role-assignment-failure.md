...
description: Enterprise runbook to resolve an application role assignment failure without skipping evidence, verification, rollback or escalation requirements.
...
- role
error_codes: []
tldr: Confirm scope and authorisation, capture evidence, isolate the failing layer, apply the least disruptive approved remediation for resolve an application role assignment failure, verify the original business task, and escalate with complete logs if recovery is not achieved.
related_symptoms:
