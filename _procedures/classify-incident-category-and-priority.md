...
description: 'Enterprise runbook to classify incident category and priority without skipping evidence, verification, rollback or escalation requirements.'
...
error_codes: []
tldr: 'Confirm scope and authorisation, capture evidence, isolate the failing layer, apply the least disruptive approved remediation for classify incident category and priority, verify the original business task, and escalate with complete logs if recovery is not achieved.'
related_symptoms:
  - classify-incident-category-and-priority
  - support-request-requires-triage
  - multiple-users-or-services-may-be-affected
symptom_weights:
  classify-incident-category-and-priority: 10
...
permalink: /procedures/classify-incident-category-and-priority/
layout: article
...
---
## Purpose and scope
Use this runbook for **classify incident category and priority** in a managed enterprise environment. It covers intake, evidence, safe diagnosis, remediation, verification, documentation and escalation. It does not replace organisation-specific security, change, safety, privacy, regulatory or vendor procedures.
...
### Targeted checks
- Confirm the exact condition described by “Classify incident category and priority” and identify its first failing dependency.
...
The exact scenario “Classify incident category and priority” is resolved and the original business task succeeds.
