...
description: 'Enterprise runbook to recover a device requesting a bitlocker recovery key without skipping evidence, verification, rollback or escalation requirements.'
...
tags:
  ...
  - windows-endpoints
error_codes: []
tldr: 'Confirm scope and authorisation, capture evidence, isolate the failing layer, apply the least disruptive approved remediation for recover a device requesting a bitlocker recovery key, verify the original business task, and escalate with complete logs if recovery is not achieved.'
related_symptoms:
...
---
## Purpose and scope
Use this runbook for **recover a device requesting a bitlocker recovery key** in a managed enterprise environment. It covers intake, evidence, safe diagnosis, remediation, verification, documentation and escalation. It does not replace organisation-specific security, change, safety, privacy, regulatory or vendor procedures.

## Mandatory Batch B-3 controls
These audit-derived controls are mandatory before more invasive remediation.

### Pre-checks
1. Step 0: verify the user's identity and the exact managed device before disclosing or using any BitLocker recovery secret.
2. Retrieve the key only from the organisation's authorised directory/MDM/escrow source.
3. Confirm the displayed recovery-key ID/device identity matches the escrowed record before use.

### Rollback / undo
- Recovery-key use is not an undoable configuration change. After recovery, investigate why recovery was triggered and rotate/re-escrow protection material where policy requires it; never expose the recovery key in tickets or chat logs.
...
### Targeted checks
- Verify device ownership, asset identity and recovery-key escrow before disclosing any key.
- Investigate why recovery was triggered, including firmware, boot, TPM or policy changes.

### Targeted remediation sequence
1. Use the approved recovery process and suspend/resume protection only when authorised.
2. Confirm encryption returns to protected state after recovery.
...
## Procedure
0. **Verify identity and exact device before using a recovery key.** Match the managed device and recovery-key ID to the authorised escrow record; never copy the secret into the ticket.
