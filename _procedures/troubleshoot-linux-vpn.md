...
description: 'Enterprise runbook to troubleshoot linux vpn without skipping evidence, verification, rollback or escalation requirements.'
...
tags:
  ...
  - vpn
error_codes: []
tldr: 'Confirm scope and authorisation, capture evidence, isolate the failing layer, apply the least disruptive approved remediation for troubleshoot linux vpn, verify the original business task, and escalate with complete logs if recovery is not achieved.'
related_symptoms:
  - linux-vpn
...
### Pre-checks
1. Confirm gateway reachability and DNS resolution from the Linux endpoint.
2. Review journal/system/client VPN logs before changing the client.
3. Confirm system date/time and NTP synchronisation.

### Rollback / undo
- Record the current approved VPN client package/profile and restore the previous approved version/profile through package management or software distribution if the change causes regression.
