---
title: Troubleshoot a service account
slug: troubleshoot-a-service-account
description: 'Enterprise runbook to troubleshoot a service account without skipping evidence, verification, rollback or escalation requirements.'
...
tags:
  ...
  - troubleshoot
error_codes: []
tldr: 'Confirm scope and authorisation, capture evidence, isolate the failing layer, apply the least disruptive approved remediation for troubleshoot a service account, verify the original business task, and escalate with complete logs if recovery is not achieved.'
related_symptoms:
  - a-service-account
  - cannot-sign-in
  - access-is-denied-or-missing
symptom_weights:
  a-service-account: 10
  cannot-sign-in: 3
...
permalink: /procedures/troubleshoot-a-service-account/
layout: article
...
### Targeted checks
- Confirm the exact condition described by “Troubleshoot a service account” and identify its first failing dependency.
...
The exact scenario “Troubleshoot a service account” is resolved and the original business task succeeds.
...
4. **Inspect the service account and its service principal names without changing credentials.** Confirm account state, password policy, SPNs and dependent services or tasks.

{% capture enterprise_command %}
Get-ADUser -Identity "service_account" -Properties Enabled,LockedOut,PasswordLastSet,PasswordNeverExpires,LastLogonDate,ServicePrincipalName |
  Select-Object SamAccountName,Enabled,LockedOut,PasswordLastSet,PasswordNeverExpires,LastLogonDate,ServicePrincipalName

setspn.exe -L service_account
{% endcapture %}
{% include command.html shell="powershell" label="Service-account evidence" command=enterprise_command %}

   <div class="expected"><strong>Expected result:</strong> The account state, password timing and SPN registrations can be correlated with the failing service without resetting the credential prematurely.</div>
