---
title: Respond to an expiring TLS certificate
slug: respond-to-an-expiring-tls-certificate
description: Enterprise runbook to respond to an expiring tls certificate without skipping evidence, verification, rollback or escalation requirements.
...
- to
error_codes: []
tldr: Confirm scope and authorisation, capture evidence, isolate the failing layer, apply the least disruptive approved remediation for respond to an expiring tls certificate, verify the original business task, and escalate with complete logs if recovery is not achieved.
related_symptoms:
- reported-an-expiring-tls-certificate
- a-server-or-infrastructure-service-is-degraded
symptom_weights:
  reported-an-expiring-tls-certificate: 10
...
permalink: /procedures/respond-to-an-expiring-tls-certificate/
layout: article
...
---
## Purpose and scope
Use this runbook for **respond to an expiring tls certificate** in a managed enterprise environment. It covers intake, evidence, safe diagnosis, remediation, verification, documentation and escalation. It does not replace organisation-specific security, change, safety, privacy, regulatory or vendor procedures.
...
### Targeted checks
- Confirm the exact condition described by “Respond to an expiring TLS certificate” and identify its first failing dependency.
...
The exact scenario “Respond to an expiring TLS certificate” is resolved and the original business task succeeds.
