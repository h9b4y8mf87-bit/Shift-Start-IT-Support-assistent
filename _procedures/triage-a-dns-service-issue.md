---
title: Triage a DNS service issue
slug: triage-a-dns-service-issue
description: Enterprise runbook to triage a dns service issue without skipping evidence, verification, rollback or escalation requirements.
...
- triage
error_codes: []
tldr: Confirm scope and authorisation, capture evidence, isolate the failing layer, apply the least disruptive approved remediation for triage a dns service issue, verify the original business task, and escalate with complete logs if recovery is not achieved.
related_symptoms:
- a-dns-service-issue
- a-server-or-infrastructure-service-is-degraded
symptom_weights:
  a-dns-service-issue: 10
...
permalink: /procedures/triage-a-dns-service-issue/
layout: article
