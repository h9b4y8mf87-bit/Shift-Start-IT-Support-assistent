---
title: Resolve an expired user certificate
slug: resolve-an-expired-user-certificate
description: 'Enterprise runbook to resolve an expired user certificate without skipping evidence, verification, rollback or escalation requirements.'
...
tags:
  ...
  - user
error_codes: []
tldr: 'Confirm scope and authorisation, capture evidence, isolate the failing layer, apply the least disruptive approved remediation for resolve an expired user certificate, verify the original business task, and escalate with complete logs if recovery is not achieved.'
related_symptoms:
  - an-expired-user-certificate
  - cannot-sign-in
  - access-is-denied-or-missing
symptom_weights:
  an-expired-user-certificate: 10
  cannot-sign-in: 3
...
permalink: /procedures/resolve-an-expired-user-certificate/
layout: article
...
---
## Purpose and scope
Use this runbook for **resolve an expired user certificate** in a managed enterprise environment. It covers intake, evidence, safe diagnosis, remediation, verification, documentation and escalation. It does not replace organisation-specific security, change, safety, privacy, regulatory or vendor procedures.
...
### Targeted checks
- Confirm the exact condition described by “Resolve an expired user certificate” and identify its first failing dependency.
...
The exact scenario “Resolve an expired user certificate” is resolved and the original business task succeeds.
