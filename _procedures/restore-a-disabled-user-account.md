---
title: Restore a disabled user account
slug: restore-a-disabled-user-account
description: 'Enterprise runbook to restore a disabled user account without skipping evidence, verification, rollback or escalation requirements.'
...
tags:
  ...
  - user
error_codes: []
tldr: 'Confirm scope and authorisation, capture evidence, isolate the failing layer, apply the least disruptive approved remediation for restore a disabled user account, verify the original business task, and escalate with complete logs if recovery is not achieved.'
related_symptoms:
  - a-disabled-user-account
  - cannot-sign-in
  - access-is-denied-or-missing
symptom_weights:
  a-disabled-user-account: 10
  cannot-sign-in: 3
...
permalink: /procedures/restore-a-disabled-user-account/
layout: article
...
---
## Purpose and scope
Use this runbook for **restore a disabled user account** in a managed enterprise environment. It covers intake, evidence, safe diagnosis, remediation, verification, documentation and escalation. It does not replace organisation-specific security, change, safety, privacy, regulatory or vendor procedures.
...
### Targeted checks
- Confirm HR or manager authorisation, effective time and legal-hold requirements.
- Inventory sessions, devices, groups, delegated access, mailboxes, licences and privileged roles.

### Targeted remediation sequence
1. Apply the approved disable/revoke sequence and transfer ownership before removing access.
2. Do not delete identities or data until retention requirements are confirmed.
