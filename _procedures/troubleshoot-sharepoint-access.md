---
title: Troubleshoot SharePoint access
slug: troubleshoot-sharepoint-access
description: 'Enterprise runbook to troubleshoot sharepoint access without skipping evidence, verification, rollback or escalation requirements.'
...
tags:
  ...
  - troubleshoot
error_codes: []
tldr: 'Confirm scope and authorisation, capture evidence, isolate the failing layer, apply the least disruptive approved remediation for troubleshoot sharepoint access, verify the original business task, and escalate with complete logs if recovery is not achieved.'
related_symptoms:
  - sharepoint-access
  - microsoft-365-feature-is-not-working
  - cloud-collaboration-access-is-failing
symptom_weights:
  sharepoint-access: 10
...
permalink: /procedures/troubleshoot-sharepoint-access/
layout: article
...
---
## Purpose and scope
Use this runbook for **troubleshoot sharepoint access** in a managed enterprise environment. It covers intake, evidence, safe diagnosis, remediation, verification, documentation and escalation. It does not replace organisation-specific security, change, safety, privacy, regulatory or vendor procedures.

## Mandatory Batch B-6 controls
These audit-derived controls are mandatory before more invasive remediation.

### Pre-checks
1. Check Microsoft 365/SharePoint service health and whether the issue affects one user, one site/library or multiple users.
2. Verify site/group/permission state and whether the site is locked/read-only or access is restricted by Conditional Access/sharing policy.
3. Test SharePoint web access from a clean browser session before changing permissions.

### Rollback / undo
- Capture existing permissions/site policy before change. Restore the previous approved permission/policy if an authorised change broadens or removes access incorrectly.
