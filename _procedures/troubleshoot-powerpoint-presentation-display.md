---
title: Troubleshoot PowerPoint presentation display
slug: troubleshoot-powerpoint-presentation-display
description: 'Enterprise runbook to troubleshoot powerpoint presentation display without skipping evidence, verification, rollback or escalation requirements.'
...
tags:
  ...
  - troubleshoot
error_codes: []
tldr: 'Confirm scope and authorisation, capture evidence, isolate the failing layer, apply the least disruptive approved remediation for troubleshoot powerpoint presentation display, verify the original business task, and escalate with complete logs if recovery is not achieved.'
related_symptoms:
  - powerpoint-presentation-display
  - microsoft-365-feature-is-not-working
  - cloud-collaboration-access-is-failing
symptom_weights:
  powerpoint-presentation-display: 10
...
permalink: /procedures/troubleshoot-powerpoint-presentation-display/
layout: article
...
---
## Purpose and scope
Use this runbook for **troubleshoot powerpoint presentation display** in a managed enterprise environment. It covers intake, evidence, safe diagnosis, remediation, verification, documentation and escalation. It does not replace organisation-specific security, change, safety, privacy, regulatory or vendor procedures.

## Mandatory Batch B-6 controls
These audit-derived controls are mandatory before more invasive remediation.

### Pre-checks
1. Confirm the external display/projector is detected and works outside PowerPoint.
2. Check Duplicate/Extend mode, Presenter View, slide-show monitor selection and supported resolution/scaling.
3. Test the presentation on the primary display or another approved output path to isolate PowerPoint from OS/display hardware.

### Rollback / undo
- Record display/PowerPoint settings before change. Restore the previous approved display/graphics configuration if a confirmed change caused regression; use supported driver rollback only where available.
