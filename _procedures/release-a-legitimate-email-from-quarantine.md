...
description: 'Enterprise runbook to release a legitimate email from quarantine without skipping evidence, verification, rollback or escalation requirements.'
...
tags:
  ...
  - release
error_codes: []
tldr: 'Confirm scope and authorisation, capture evidence, isolate the failing layer, apply the least disruptive approved remediation for release a legitimate email from quarantine, verify the original business task, and escalate with complete logs if recovery is not achieved.'
related_symptoms:
