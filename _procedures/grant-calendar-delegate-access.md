---
title: Grant calendar delegate access
slug: grant-calendar-delegate-access
description: 'Enterprise runbook to grant calendar delegate access without skipping evidence, verification, rollback or escalation requirements.'
...
tags:
  ...
  - l1-l2
error_codes: []
tldr: 'Confirm scope and authorisation, capture evidence, isolate the failing layer, apply the least disruptive approved remediation for grant calendar delegate access, verify the original business task, and escalate with complete logs if recovery is not achieved.'
related_symptoms:
  - need-calendar-delegate-access
  - cannot-sign-in
  - access-is-denied-or-missing
symptom_weights:
  need-calendar-delegate-access: 10
  cannot-sign-in: 3
...
permalink: /procedures/grant-calendar-delegate-access/
layout: article
...
---
## Purpose and scope
Use this runbook for **grant calendar delegate access** in a managed enterprise environment. It covers intake, evidence, safe diagnosis, remediation, verification, documentation and escalation. It does not replace organisation-specific security, change, safety, privacy, regulatory or vendor procedures.
...
### Targeted checks
- Confirm the exact condition described by “Grant calendar delegate access” and identify its first failing dependency.
...
The exact scenario “Grant calendar delegate access” is resolved and the original business task succeeds.
...
4. **Inspect existing calendar permissions before granting access.** Connect to Exchange Online with an approved administrative role and confirm whether the delegate already has an entry and which access level is currently effective.

{% capture enterprise_command %}
Connect-ExchangeOnline
Get-MailboxFolderPermission -Identity "owner@contoso.com:\Calendar" |
  Select-Object User,AccessRights,SharingPermissionFlags
{% endcapture %}
{% include command.html shell="powershell" label="Calendar permission evidence" command=enterprise_command %}

   <div class="expected"><strong>Expected result:</strong> The existing calendar ACL is captured, duplicate or conflicting entries are identified, and the requested delegate level can be compared with the approved request.</div>
