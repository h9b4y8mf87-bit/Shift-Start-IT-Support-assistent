...
description: Enterprise runbook to isolate an endpoint through approved edr controls without skipping evidence, verification, rollback or escalation requirements.
...
- through
error_codes: []
tldr: Confirm scope and authorisation, capture evidence, isolate the failing layer, apply the least disruptive approved remediation for isolate an endpoint through approved edr controls, verify the original business task, and escalate with complete logs if recovery is not achieved.
related_symptoms:
- isolate-an-endpoint-through-approved-edr-controls
- a-security-or-compliance-concern-is-reported
symptom_weights:
  isolate-an-endpoint-through-approved-edr-controls: 10
...
permalink: /procedures/isolate-an-endpoint-through-approved-edr-controls/
layout: article
...
### Targeted checks
- Confirm the exact condition described by “Isolate an endpoint through approved EDR controls” and identify its first failing dependency.
...
The exact scenario “Isolate an endpoint through approved EDR controls” is resolved and the original business task succeeds.
