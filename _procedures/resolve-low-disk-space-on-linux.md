...
description: Enterprise runbook to resolve low disk space on linux without skipping evidence, verification, rollback
...
tldr: Confirm scope and authorisation, capture evidence, isolate the failing layer, apply the least disruptive approved
  remediation for resolve low disk space on linux, verify the original business task, and escalate with complete
