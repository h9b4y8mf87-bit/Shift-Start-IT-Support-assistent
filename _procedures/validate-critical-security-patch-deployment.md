...
description: 'Enterprise runbook to validate critical security patch deployment without skipping evidence, verification, rollback or escalation requirements.'
...
tags:
  ...
  - validate
error_codes: []
tldr: 'Confirm scope and authorisation, capture evidence, isolate the failing layer, apply the least disruptive approved remediation for validate critical security patch deployment, verify the original business task, and escalate with complete logs if recovery is not achieved.'
related_symptoms:
