...
description: Enterprise runbook to collect server logs for escalation without skipping evidence, verification, rollback
...
tldr: Confirm scope and authorisation, capture evidence, isolate the failing layer, apply the least disruptive approved
  remediation for collect server logs for escalation, verify the original business task, and escalate with complete
