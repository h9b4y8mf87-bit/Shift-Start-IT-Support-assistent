...
description: Enterprise runbook to troubleshoot an application that will not install without skipping evidence,
...
tldr: Confirm scope and authorisation, capture evidence, isolate the failing layer, apply the least disruptive approved
  remediation for troubleshoot an application that will not install, verify the original business task, and escalate
  with complete logs if recovery is not achieved.
related_symptoms:
