---
title: Provision external guest access
slug: provision-external-guest-access
description: 'Enterprise runbook to provision external guest access without skipping evidence, verification, rollback or escalation requirements.'
...
tags:
  ...
  - provision
error_codes: []
tldr: 'Confirm scope and authorisation, capture evidence, isolate the failing layer, apply the least disruptive approved remediation for provision external guest access, verify the original business task, and escalate with complete logs if recovery is not achieved.'
related_symptoms:
  - need-to-provision-external-guest-access
  - cannot-sign-in
  - access-is-denied-or-missing
symptom_weights:
  need-to-provision-external-guest-access: 10
  cannot-sign-in: 3
...
permalink: /procedures/provision-external-guest-access/
layout: article
...
---
## Purpose and scope
Use this runbook for **provision external guest access** in a managed enterprise environment. It covers intake, evidence, safe diagnosis, remediation, verification, documentation and escalation. It does not replace organisation-specific security, change, safety, privacy, regulatory or vendor procedures.

## Mandatory Batch B-3 controls
These audit-derived controls are mandatory before more invasive remediation.

### Pre-checks
1. Verify the guest user's email address and identity.
2. Confirm the sponsor/manager has approved the access request.
3. Check the required sharing policy and group permissions to avoid over-privileging.

### Rollback / undo
- Immediately remove the guest user from Entra ID/B2B if access is no longer required or was granted in error.
