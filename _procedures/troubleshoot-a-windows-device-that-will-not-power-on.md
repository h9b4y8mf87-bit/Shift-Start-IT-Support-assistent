---
title: Troubleshoot a Windows device that will not power on
slug: troubleshoot-a-windows-device-that-will-not-power-on
description: 'Enterprise runbook to troubleshoot a windows device that will not power on without skipping evidence, verification, rollback or escalation requirements.'
...
tags:
  ...
  - windows-endpoints
error_codes: []
tldr: 'Confirm scope and authorisation, capture evidence, isolate the failing layer, apply the least disruptive approved remediation for troubleshoot a windows device that will not power on, verify the original business task, and escalate with complete logs if recovery is not achieved.'
related_symptoms:
  - a-windows-device-that-will-not-power-on
  - windows-device-is-not-working-correctly
  - a-windows-error-or-performance-issue-is-reported
symptom_weights:
  a-windows-device-that-will-not-power-on: 10
...
permalink: /procedures/troubleshoot-a-windows-device-that-will-not-power-on/
layout: article
...
---
## Purpose and scope
Use this runbook for **troubleshoot a windows device that will not power on** in a managed enterprise environment. It covers intake, evidence, safe diagnosis, remediation, verification, documentation and escalation. It does not replace organisation-specific security, change, safety, privacy, regulatory or vendor procedures.

## Mandatory Batch B-4 controls
These audit-derived controls are mandatory before more invasive remediation.

### Pre-checks
1. Check known-good power adapter/cable/dock and battery/charging indicators before opening hardware.
2. Disconnect nonessential peripherals and perform the vendor-approved power reset.
3. Check POST/diagnostic LED/beep codes and stop for battery swelling, liquid, overheating, smoke or electrical damage.

### Rollback / undo
- Hardware power diagnosis generally has no software rollback. If a firmware/BIOS update is confirmed causal, use only the vendor-supported recovery mechanism; otherwise route to authorised hardware repair/replacement.
...
### Targeted checks
- Confirm the exact condition described by “Troubleshoot a Windows device that will not power on” and identify its first failing dependency.
...
The exact scenario “Troubleshoot a Windows device that will not power on” is resolved and the original business task succeeds.
