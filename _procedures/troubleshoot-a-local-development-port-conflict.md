---
title: Troubleshoot a local development port conflict
slug: troubleshoot-a-local-development-port-conflict
description: 'Enterprise runbook to troubleshoot a local development port conflict without skipping evidence, verification, rollback or escalation requirements.'
...
tags:
  ...
  - troubleshoot
error_codes: []
tldr: 'Confirm scope and authorisation, capture evidence, isolate the failing layer, apply the least disruptive approved remediation for troubleshoot a local development port conflict, verify the original business task, and escalate with complete logs if recovery is not achieved.'
related_symptoms:
  - a-local-development-port-conflict
  - linux-or-developer-tooling-is-not-working
symptom_weights:
  a-local-development-port-conflict: 10
...
permalink: /procedures/troubleshoot-a-local-development-port-conflict/
layout: article
...
---
## Purpose and scope
Use this runbook for **troubleshoot a local development port conflict** in a managed enterprise environment. It covers intake, evidence, safe diagnosis, remediation, verification, documentation and escalation. It does not replace organisation-specific security, change, safety, privacy, regulatory or vendor procedures.

## Mandatory Batch B-4 controls
These audit-derived controls are mandatory before more invasive remediation.

### Pre-checks
1. Run `netstat -ano` to identify the process using the conflicted port.
2. Check if the conflicting process can be stopped or reconfigured.
3. Verify the developer has administrative rights to stop the offending service.

### Rollback / undo
- If you stop a critical process to free the port, restart it immediately after the conflicting application is re-configured.
