---
title: Troubleshoot an ERP client
slug: troubleshoot-an-erp-client
description: 'Enterprise runbook to troubleshoot an erp client without skipping evidence, verification, rollback or escalation requirements.'
...
tags:
  ...
  - troubleshoot
error_codes: []
tldr: 'Confirm scope and authorisation, capture evidence, isolate the failing layer, apply the least disruptive approved remediation for troubleshoot an erp client, verify the original business task, and escalate with complete logs if recovery is not achieved.'
related_symptoms:
  - an-erp-client
...
permalink: /procedures/troubleshoot-an-erp-client/
layout: article
...
---
## Purpose and scope
Use this runbook for **troubleshoot an erp client** in a managed enterprise environment. It covers intake, evidence, safe diagnosis, remediation, verification, documentation and escalation. It does not replace organisation-specific security, change, safety, privacy, regulatory or vendor procedures.

## Mandatory Batch B-4 controls
These audit-derived controls are mandatory before more invasive remediation.

### Pre-checks
1. Determine whether the problem is client-specific or affects multiple users/sites.
2. Check ERP application/server/database dependency health and relevant connection/session limits.
3. Verify user role/authorisation and recent client/server changes before reinstalling the client.

### Rollback / undo
- Record the current supported ERP client/configuration. If a confirmed client patch/configuration change caused failure, deploy the previous approved version/configuration through the enterprise software/ERP management process.
