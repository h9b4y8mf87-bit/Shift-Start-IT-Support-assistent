---
title: Triage a Windows Server that is unreachable
slug: triage-a-windows-server-that-is-unreachable
description: Enterprise runbook to triage a windows server that is unreachable without skipping evidence, verification, rollback or escalation requirements.
...
- windows
error_codes: []
tldr: Confirm scope and authorisation, capture evidence, isolate the failing layer, apply the least disruptive approved remediation for triage a windows server that is unreachable, verify the original business task, and escalate with complete logs if recovery is not achieved.
related_symptoms:
- a-windows-server-that-is-unreachable
- a-server-or-infrastructure-service-is-degraded
symptom_weights:
  a-windows-server-that-is-unreachable: 10
...
permalink: /procedures/triage-a-windows-server-that-is-unreachable/
layout: article
