...
description: Enterprise runbook to triage a windows server that is unreachable without skipping evidence, verification,
...
tldr: Confirm scope and authorisation, capture evidence, isolate the failing layer, apply the least disruptive approved
  remediation for triage a windows server that is unreachable, verify the original business task, and escalate with
