...
description: Enterprise runbook to perform a desk move without skipping evidence, verification, rollback or escalation
  requirements.
...
tldr: Confirm scope and authorisation, capture evidence, isolate the failing layer, apply the least disruptive approved
  remediation for perform a desk move, verify the original business task, and escalate with complete logs if recovery
  is not achieved.
related_symptoms:
- need-to-a-desk-move
