---
title: Troubleshoot a webcam on Windows
slug: troubleshoot-a-webcam-on-windows
description: 'Enterprise runbook to troubleshoot a webcam on windows without skipping evidence, verification, rollback or escalation requirements.'
...
tags:
  ...
  - windows-endpoints
error_codes: []
tldr: 'Confirm scope and authorisation, capture evidence, isolate the failing layer, apply the least disruptive approved remediation for troubleshoot a webcam on windows, verify the original business task, and escalate with complete logs if recovery is not achieved.'
related_symptoms:
  - a-webcam-on-windows
  - windows-device-is-not-working-correctly
  - a-windows-error-or-performance-issue-is-reported
symptom_weights:
  a-webcam-on-windows: 10
...
permalink: /procedures/troubleshoot-a-webcam-on-windows/
layout: article
...
risk_basis: 'Batch B-4 audit classification: P3 / low.'
...
---
## Purpose and scope
Use this runbook for **troubleshoot a webcam on windows** in a managed enterprise environment. It covers intake, evidence, safe diagnosis, remediation, verification, documentation and escalation. It does not replace organisation-specific security, change, safety, privacy, regulatory or vendor procedures.

## Mandatory Batch B-4 controls
These audit-derived controls are mandatory before more invasive remediation.

### Pre-checks
1. Test the camera in the Windows Camera app to separate OS/hardware failure from Teams/application failure.
2. Check Windows camera privacy permissions and Device Manager status.
3. Check whether another process has exclusive camera use and test a known-good USB/internal camera path where applicable.

### Rollback / undo
- Record the existing driver/device settings. Restore the previous approved driver/configuration if an authorised driver change causes regression.
