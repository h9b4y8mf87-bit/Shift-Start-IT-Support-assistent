---
title: Recover access after a lost security key
slug: recover-access-after-a-lost-security-key
description: 'Enterprise runbook to recover access after a lost security key without skipping evidence, verification, rollback or escalation requirements.'
...
tags:
  ...
  - security
error_codes: []
tldr: 'Confirm scope and authorisation, capture evidence, isolate the failing layer, apply the least disruptive approved remediation for recover access after a lost security key, verify the original business task, and escalate with complete logs if recovery is not achieved.'
related_symptoms:
  - access-after-a-lost-security-key
  - cannot-sign-in
  - access-is-denied-or-missing
symptom_weights:
  access-after-a-lost-security-key: 10
  cannot-sign-in: 3
...
permalink: /procedures/recover-access-after-a-lost-security-key/
layout: article
...
---
## Purpose and scope
Use this runbook for **recover access after a lost security key** in a managed enterprise environment. It covers intake, evidence, safe diagnosis, remediation, verification, documentation and escalation. It does not replace organisation-specific security, change, safety, privacy, regulatory or vendor procedures.

## Mandatory Batch B-3 controls
These audit-derived controls are mandatory before more invasive remediation.

### Pre-checks
1. Step 0: Verify the user's identity via out-of-band methods (e.g., manager approval, video call).
2. Revoke the lost key from Entra ID to prevent its use by a finder.
3. Register a new authentication method (Authenticator app, SMS, etc.) for the user.

### Rollback / undo
- If the newly registered MFA method fails, remove it and re-register. Document the revocation of the lost key.
