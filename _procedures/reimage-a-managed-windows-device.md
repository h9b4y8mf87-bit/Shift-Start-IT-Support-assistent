...
description: 'Enterprise runbook to reimage a managed windows device without skipping evidence, verification, rollback or escalation requirements.'
...
tags:
  ...
  - windows-endpoints
error_codes: []
tldr: 'Confirm scope and authorisation, capture evidence, isolate the failing layer, apply the least disruptive approved remediation for reimage a managed windows device, verify the original business task, and escalate with complete logs if recovery is not achieved.'
related_symptoms:
  - reimage-a-managed-windows-device
  - windows-device-is-not-working-correctly
  - a-windows-error-or-performance-issue-is-reported
symptom_weights:
  reimage-a-managed-windows-device: 10
...
permalink: /procedures/reimage-a-managed-windows-device/
layout: article
...
---
## Purpose and scope
Use this runbook for **reimage a managed windows device** in a managed enterprise environment. It covers intake, evidence, safe diagnosis, remediation, verification, documentation and escalation. It does not replace organisation-specific security, change, safety, privacy, regulatory or vendor procedures.
...
### Targeted checks
- Confirm the exact condition described by “Reimage a managed Windows device” and identify its first failing dependency.
...
The exact scenario “Reimage a managed Windows device” is resolved and the original business task succeeds.
