---
title: Troubleshoot an application that crashes
slug: troubleshoot-an-application-that-crashes
description: 'Enterprise runbook to troubleshoot an application that crashes without skipping evidence, verification, rollback or escalation requirements.'
...
tags:
  ...
  - windows-endpoints
error_codes: []
tldr: 'Confirm scope and authorisation, capture evidence, isolate the failing layer, apply the least disruptive approved remediation for troubleshoot an application that crashes, verify the original business task, and escalate with complete logs if recovery is not achieved.'
related_symptoms:
  - an-application-that-crashes
  - windows-device-is-not-working-correctly
  - a-windows-error-or-performance-issue-is-reported
symptom_weights:
  an-application-that-crashes: 10
...
permalink: /procedures/troubleshoot-an-application-that-crashes/
layout: article
...
---
## Purpose and scope
Use this runbook for **troubleshoot an application that crashes** in a managed enterprise environment. It covers intake, evidence, safe diagnosis, remediation, verification, documentation and escalation. It does not replace organisation-specific security, change, safety, privacy, regulatory or vendor procedures.
...
### Targeted checks
- Reproduce in safe mode or without add-ins and capture crash module/version.
- Check compatibility, updates, profile/cache and a second user or device.

### Targeted remediation sequence
1. Disable only the confirmed add-in or repair the supported client.
2. Preserve user data before resetting profiles or reinstalling.

### Scenario-specific success criterion
The application opens and completes the original workflow repeatedly.
