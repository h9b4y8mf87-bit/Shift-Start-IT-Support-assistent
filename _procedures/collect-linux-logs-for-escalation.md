...
description: Enterprise runbook to collect linux logs for escalation without skipping evidence, verification, rollback
...
tldr: Confirm scope and authorisation, capture evidence, isolate the failing layer, apply the least disruptive approved
  remediation for collect linux logs for escalation, verify the original business task, and escalate with complete
...
approval_required: Follow organisational policy for privileged, destructive, security-sensitive or service-impacting
  actions.
permalink: /procedures/collect-linux-logs-for-escalation/
layout: article
---
## Purpose and scope
