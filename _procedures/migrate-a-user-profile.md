...
description: Enterprise runbook to migrate a user profile without skipping evidence, verification, rollback or escalation
  requirements.
...
tldr: Confirm scope and authorisation, capture evidence, isolate the failing layer, apply the least disruptive approved
  remediation for migrate a user profile, verify the original business task, and escalate with complete logs if
...
approval_required: Follow organisational policy for privileged, destructive, security-sensitive or service-impacting
  actions.
permalink: /procedures/migrate-a-user-profile/
layout: article
---
## Purpose and scope
