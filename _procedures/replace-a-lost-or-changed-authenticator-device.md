---
title: Replace a lost or changed authenticator device
slug: replace-a-lost-or-changed-authenticator-device
description: Enterprise runbook to replace a lost or changed authenticator device without skipping evidence, verification, rollback or escalation requirements.
...
- replace
error_codes: []
tldr: Confirm scope and authorisation, capture evidence, isolate the failing layer, apply the least disruptive approved remediation for replace a lost or changed authenticator device, verify the original business task, and escalate with complete logs if recovery is not achieved.
related_symptoms:
- replace-a-lost-or-changed-authenticator-device
- cannot-sign-in
- access-is-denied-or-missing
symptom_weights:
  replace-a-lost-or-changed-authenticator-device: 10
  cannot-sign-in: 3
...
permalink: /procedures/replace-a-lost-or-changed-authenticator-device/
layout: article
...
4. **Inventory the registered authentication methods and active sessions.** Confirm the user identity through the approved verification process before removing or replacing any method.
...
Get-MgAuditLogSignIn -Filter "userPrincipalName eq 'user@contoso.com'" -Top 20 |
  Select-Object CreatedDateTime,AppDisplayName,AuthenticationRequirement,Status
{% endcapture %}
{% include command.html shell="powershell" label="Authentication method inventory" command=enterprise_command %}

   <div class="expected"><strong>Expected result:</strong> The lost device’s method can be distinguished from remaining recovery methods and recent sign-in activity before any removal is approved.</div>
