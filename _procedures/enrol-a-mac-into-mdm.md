...
description: Enterprise runbook to enrol a mac into mdm without skipping evidence, verification, rollback or escalation
  requirements.
...
tldr: Confirm scope and authorisation, capture evidence, isolate the failing layer, apply the least disruptive approved
  remediation for enrol a mac into mdm, verify the original business task, and escalate with complete logs if recovery
  is not achieved.
related_symptoms:
