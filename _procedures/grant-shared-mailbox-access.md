---
title: Grant shared mailbox access
slug: grant-shared-mailbox-access
description: 'Enterprise runbook to grant shared mailbox access without skipping evidence, verification, rollback or escalation requirements.'
...
tags:
  ...
  - shared
error_codes: []
tldr: 'Confirm scope and authorisation, capture evidence, isolate the failing layer, apply the least disruptive approved remediation for grant shared mailbox access, verify the original business task, and escalate with complete logs if recovery is not achieved.'
related_symptoms:
  - need-shared-mailbox-access
  - cannot-sign-in
  - access-is-denied-or-missing
symptom_weights:
  need-shared-mailbox-access: 10
  cannot-sign-in: 3
...
permalink: /procedures/grant-shared-mailbox-access/
layout: article
...
---
## Purpose and scope
Use this runbook for **grant shared mailbox access** in a managed enterprise environment. It covers intake, evidence, safe diagnosis, remediation, verification, documentation and escalation. It does not replace organisation-specific security, change, safety, privacy, regulatory or vendor procedures.

## Mandatory Batch B-3 controls
These audit-derived controls are mandatory before more invasive remediation.

### Pre-checks
1. Verify the requestor has manager approval (or is the mailbox owner).
2. Confirm the shared mailbox exists and is licensed correctly.
3. Determine if the user needs Full Access, Send As, or Send on Behalf permissions.

### Rollback / undo
- Immediately revoke the permission via Exchange Admin Center if the user's access is mistakenly granted. Document the removal.
...
4. **Inspect the shared mailbox and all existing delegate permissions.** Confirm whether Full Access, Send As or Send on Behalf is requested; these are separate permissions and must not be assumed equivalent.
...
{% endcapture %}
{% include command.html shell="powershell" label="Shared mailbox permission evidence" command=enterprise_command %}

   <div class="expected"><strong>Expected result:</strong> Existing Full Access and Send As entries are recorded so only the approved permission type is added, changed or removed.</div>
