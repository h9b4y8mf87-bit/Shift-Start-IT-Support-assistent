...
description: Enterprise runbook to troubleshoot ssh access without skipping evidence, verification, rollback or
...
tldr: Confirm scope and authorisation, capture evidence, isolate the failing layer, apply the least disruptive approved
  remediation for troubleshoot ssh access, verify the original business task, and escalate with complete logs if
