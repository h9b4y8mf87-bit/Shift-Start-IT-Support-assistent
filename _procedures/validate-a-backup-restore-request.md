...
description: 'Enterprise runbook to validate a backup restore request without skipping evidence, verification, rollback or escalation requirements.'
...
tags:
  ...
  - validate
error_codes: []
tldr: 'Confirm scope and authorisation, capture evidence, isolate the failing layer, apply the least disruptive approved remediation for validate a backup restore request, verify the original business task, and escalate with complete logs if recovery is not achieved.'
related_symptoms:
