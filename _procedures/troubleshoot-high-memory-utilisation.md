---
title: Troubleshoot high memory utilisation
slug: troubleshoot-high-memory-utilisation
description: 'Enterprise runbook to troubleshoot high memory utilisation without skipping evidence, verification, rollback or escalation requirements.'
...
tags:
  ...
  - windows-endpoints
error_codes: []
tldr: 'Confirm scope and authorisation, capture evidence, isolate the failing layer, apply the least disruptive approved remediation for troubleshoot high memory utilisation, verify the original business task, and escalate with complete logs if recovery is not achieved.'
related_symptoms:
  - high-memory-utilisation
  - windows-device-is-not-working-correctly
  - a-windows-error-or-performance-issue-is-reported
symptom_weights:
  high-memory-utilisation: 10
...
permalink: /procedures/troubleshoot-high-memory-utilisation/
layout: article
...
### Pre-checks
1. Determine endpoint versus server/service-wide scope and identify top processes, working set/private memory and commit/pagefile pressure.
2. Check for sustained growth/leak patterns, recent deployments and application/service errors.
3. Check available memory, paging/swap behaviour and whether memory pressure is causing process termination or service degradation.

### Rollback / undo
- If a confirmed recent deployment/configuration caused the leak, restore the previous approved release/configuration. Restart affected services only under change authority and after checking dependencies and preserving diagnostic evidence.
