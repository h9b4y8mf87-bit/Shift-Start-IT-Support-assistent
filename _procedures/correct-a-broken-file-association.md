---
title: Correct a broken file association
slug: correct-a-broken-file-association
description: 'Enterprise runbook to correct a broken file association without skipping evidence, verification, rollback or escalation requirements.'
...
tags:
  ...
  - windows-endpoints
error_codes: []
tldr: 'Confirm scope and authorisation, capture evidence, isolate the failing layer, apply the least disruptive approved remediation for correct a broken file association, verify the original business task, and escalate with complete logs if recovery is not achieved.'
related_symptoms:
  - a-broken-file-association
  - windows-device-is-not-working-correctly
  - a-windows-error-or-performance-issue-is-reported
symptom_weights:
  a-broken-file-association: 10
...
permalink: /procedures/correct-a-broken-file-association/
layout: article
...
risk_basis: 'Batch B-3 audit classification: P3 / low.'
...
---
## Purpose and scope
Use this runbook for **correct a broken file association** in a managed enterprise environment. It covers intake, evidence, safe diagnosis, remediation, verification, documentation and escalation. It does not replace organisation-specific security, change, safety, privacy, regulatory or vendor procedures.

## Mandatory Batch B-3 controls
These audit-derived controls are mandatory before more invasive remediation.

### Pre-checks
1. Verify the application is installed and functional.
2. Check if the file extension is correct (e.g., `.docx` vs `.doc`).
3. Use Windows "Set Default Programs" or `assoc`/`ftype` commands.

### Rollback / undo
- Export the current file association registry key (`HKEY_CLASSES_ROOT\.ext`) before changing it. If the change fails, re-import the registry.
