...
description: 'Enterprise runbook to check domain controller health without skipping evidence, verification, rollback or escalation requirements.'
...
error_codes: []
tldr: 'Confirm scope and authorisation, capture evidence, isolate the failing layer, apply the least disruptive approved remediation for check domain controller health, verify the original business task, and escalate with complete logs if recovery is not achieved.'
related_symptoms:
  - check-domain-controller-health
  - a-server-or-infrastructure-service-is-degraded
symptom_weights:
  check-domain-controller-health: 10
...
permalink: /procedures/check-domain-controller-health/
layout: article
...
---
## Purpose and scope
Use this runbook for **check domain controller health** in a managed enterprise environment. It covers intake, evidence, safe diagnosis, remediation, verification, documentation and escalation. It does not replace organisation-specific security, change, safety, privacy, regulatory or vendor procedures.
