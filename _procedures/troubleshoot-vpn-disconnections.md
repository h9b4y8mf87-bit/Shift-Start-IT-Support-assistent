---
title: Troubleshoot VPN disconnections
slug: troubleshoot-vpn-disconnections
description: 'Enterprise runbook to troubleshoot vpn disconnections without skipping evidence, verification, rollback or escalation requirements.'
...
tags:
  ...
  - vpn
error_codes: []
tldr: 'Confirm scope and authorisation, capture evidence, isolate the failing layer, apply the least disruptive approved remediation for troubleshoot vpn disconnections, verify the original business task, and escalate with complete logs if recovery is not achieved.'
related_symptoms:
  - vpn-disconnections
  - network-or-internet-access-is-unavailable
  - connection-is-slow-or-intermittent
symptom_weights:
  vpn-disconnections: 10
...
permalink: /procedures/troubleshoot-vpn-disconnections/
layout: article
...
---
## Purpose and scope
Use this runbook for **troubleshoot vpn disconnections** in a managed enterprise environment. It covers intake, evidence, safe diagnosis, remediation, verification, documentation and escalation. It does not replace organisation-specific security, change, safety, privacy, regulatory or vendor procedures.

## Mandatory Batch B-1 controls
These audit-derived controls are mandatory before more invasive remediation.

### Pre-checks
1. Check gateway logs for IKE/IPsec rekey, Dead Peer Detection and session-timeout failures.
2. Check address-pool/session capacity.
3. Capture packet-loss/route behaviour during the disconnect window.

### Rollback / undo
- Record existing rekey/DPD/session values and restore previous stable values if disconnect frequency worsens; use the vendor-supported rollback mechanism for remote gateway changes.
