...
description: 'Enterprise runbook to troubleshoot a legacy application compatibility issue without skipping evidence, verification, rollback or escalation requirements.'
...
tags:
  ...
  - troubleshoot
error_codes: []
tldr: 'Confirm scope and authorisation, capture evidence, isolate the failing layer, apply the least disruptive approved remediation for troubleshoot a legacy application compatibility issue, verify the original business task, and escalate with complete logs if recovery is not achieved.'
related_symptoms:
...
### Targeted checks
- Confirm the exact condition described by “Troubleshoot a legacy application compatibility issue” and identify its first failing dependency.
...
The exact scenario “Troubleshoot a legacy application compatibility issue” is resolved and the original business task succeeds.
