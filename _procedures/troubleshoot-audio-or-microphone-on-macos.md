---
title: Troubleshoot audio or microphone on macOS
slug: troubleshoot-audio-or-microphone-on-macos
description: 'Enterprise runbook to troubleshoot audio or microphone on macos without skipping evidence, verification, rollback or escalation requirements.'
...
tags:
  ...
  - troubleshoot
error_codes: []
tldr: 'Confirm scope and authorisation, capture evidence, isolate the failing layer, apply the least disruptive approved remediation for troubleshoot audio or microphone on macos, verify the original business task, and escalate with complete logs if recovery is not achieved.'
related_symptoms:
  - audio-or-microphone-on-macos
  - mac-is-not-working-correctly
  - a-macos-error-or-access-issue-is-reported
symptom_weights:
  audio-or-microphone-on-macos: 10
...
permalink: /procedures/troubleshoot-audio-or-microphone-on-macos/
layout: article
...
---
## Purpose and scope
Use this runbook for **troubleshoot audio or microphone on macos** in a managed enterprise environment. It covers intake, evidence, safe diagnosis, remediation, verification, documentation and escalation. It does not replace organisation-specific security, change, safety, privacy, regulatory or vendor procedures.

## Mandatory Batch B-5 controls
These audit-derived controls are mandatory before more invasive remediation.

### Pre-checks
1. Verify selected input/output device and volume/mute state in macOS Sound settings.
2. Check application microphone permission in Privacy & Security and test recording with a built-in supported app.
3. Test a known-good headset/microphone or alternate port/device to separate hardware from application configuration.

### Rollback / undo
- Record audio/device/application settings before change. Restore prior approved configuration if an app/MDM change caused regression. Do not attempt unsupported macOS audio-driver rollback; use supported OS/vendor remediation.
...
4. **Run non-destructive diagnostics.** Check macOS version, MDM enrolment, privacy permissions, Keychain or certificate state, Console logs, storage and recent configuration changes.

{% capture enterprise_command %}
sw_vers; system_profiler SPHardwareDataType SPSoftwareDataType | head -80; profiles status -type enrollment
{% endcapture %}
{% include command.html shell="bash" label="macOS baseline" command=enterprise_command %}
...
5. **Apply the primary approved remediation.** Correct the user-level setting or approved profile first, then restart the affected application or service.
...
7. **Apply secondary remediation only when justified.** Use the managed remediation, profile redeployment, safe-mode validation or approved macOS recovery workflow if the first repair fails.
...
9. **Complete end-to-end verification.** The affected function works for the user and the Mac remains enrolled, encrypted and compliant.
