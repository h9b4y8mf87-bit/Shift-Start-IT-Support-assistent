---
title: Resolve low disk space on Windows
slug: resolve-low-disk-space-on-windows
description: 'Enterprise runbook to resolve low disk space on windows without skipping evidence, verification, rollback or escalation requirements.'
...
tags:
  ...
  - windows-endpoints
error_codes: []
tldr: 'Confirm scope and authorisation, capture evidence, isolate the failing layer, apply the least disruptive approved remediation for resolve low disk space on windows, verify the original business task, and escalate with complete logs if recovery is not achieved.'
related_symptoms:
  - low-disk-space-on-windows
  - windows-device-is-not-working-correctly
  - a-windows-error-or-performance-issue-is-reported
symptom_weights:
  low-disk-space-on-windows: 10
...
permalink: /procedures/resolve-low-disk-space-on-windows/
layout: article
...
---
## Purpose and scope
Use this runbook for **resolve low disk space on windows** in a managed enterprise environment. It covers intake, evidence, safe diagnosis, remediation, verification, documentation and escalation. It does not replace organisation-specific security, change, safety, privacy, regulatory or vendor procedures.
...
### Targeted checks
- Identify the consuming volume, folder, mailbox or quota and its growth trend.
- Separate business data, cache, logs, temporary files and recoverable content.

### Targeted remediation sequence
1. Remove or archive only approved data; expand capacity through the owning team when justified.
2. Do not delete logs, backups, legal-hold data or user files without approval.

### Scenario-specific success criterion
Required free capacity is restored and growth is controlled.
