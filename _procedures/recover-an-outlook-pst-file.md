...
description: Enterprise runbook to recover an outlook pst file without skipping evidence, verification, rollback
...
tldr: Confirm scope and authorisation, capture evidence, isolate the failing layer, apply the least disruptive approved
  remediation for recover an outlook pst file, verify the original business task, and escalate with complete logs
  if recovery is not achieved.
related_symptoms:
- an-outlook-pst-file
- business-data-is-missing-corrupted-or-requires-recovery
symptom_weights:
  an-outlook-pst-file: 10
