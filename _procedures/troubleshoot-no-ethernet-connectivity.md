---
title: Troubleshoot no Ethernet connectivity
slug: troubleshoot-no-ethernet-connectivity
description: 'Enterprise runbook to troubleshoot no ethernet connectivity without skipping evidence, verification, rollback or escalation requirements.'
...
tags:
  ...
  - troubleshoot
error_codes: []
tldr: 'Confirm scope and authorisation, capture evidence, isolate the failing layer, apply the least disruptive approved remediation for troubleshoot no ethernet connectivity, verify the original business task, and escalate with complete logs if recovery is not achieved.'
related_symptoms:
  - no-ethernet-connectivity
  - network-or-internet-access-is-unavailable
  - connection-is-slow-or-intermittent
symptom_weights:
  no-ethernet-connectivity: 10
...
permalink: /procedures/troubleshoot-no-ethernet-connectivity/
layout: article
...
---
## Purpose and scope
Use this runbook for **troubleshoot no ethernet connectivity** in a managed enterprise environment. It covers intake, evidence, safe diagnosis, remediation, verification, documentation and escalation. It does not replace organisation-specific security, change, safety, privacy, regulatory or vendor procedures.

## Mandatory Batch B-2 controls
These audit-derived controls are mandatory before more invasive remediation.

### Pre-checks
1. Check cable/link LEDs, NIC state and physical port before changing software.
2. Check switch-port VLAN/access configuration and port-security state.
3. Check STP, errors and MAC-learning evidence when the fault points to the switch rather than the endpoint.

### Rollback / undo
- Capture the current switch-port configuration before a change. Restore the prior port configuration if connectivity worsens; use the platform's supported remote rollback safety mechanism for switch changes.
...
### Targeted checks
- Check adapter/link, address, mask, gateway, DNS, lease, route and authentication in order.
- Compare another device on the same port, SSID or VLAN.

### Targeted remediation sequence
1. Renew or correct local configuration only after capturing it; reconnect with approved credentials/profile.
2. Escalate port, scope, VLAN, DNS or infrastructure evidence when multiple devices are affected.
