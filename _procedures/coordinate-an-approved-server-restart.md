...
description: Enterprise runbook to coordinate an approved server restart without skipping evidence, verification,
...
tldr: Confirm scope and authorisation, capture evidence, isolate the failing layer, apply the least disruptive approved
  remediation for coordinate an approved server restart, verify the original business task, and escalate with complete
