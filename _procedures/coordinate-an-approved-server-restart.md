---
title: Coordinate an approved server restart
slug: coordinate-an-approved-server-restart
description: Enterprise runbook to coordinate an approved server restart without skipping evidence, verification, rollback or escalation requirements.
...
error_codes: []
tldr: Confirm scope and authorisation, capture evidence, isolate the failing layer, apply the least disruptive approved remediation for coordinate an approved server restart, verify the original business task, and escalate with complete logs if recovery is not achieved.
related_symptoms:
- need-to-coordinate-an-approved-server-restart
- a-server-or-infrastructure-service-is-degraded
symptom_weights:
  need-to-coordinate-an-approved-server-restart: 10
...
permalink: /procedures/coordinate-an-approved-server-restart/
layout: article
...
---
## Purpose and scope
Use this runbook for **coordinate an approved server restart** in a managed enterprise environment. It covers intake, evidence, safe diagnosis, remediation, verification, documentation and escalation. It does not replace organisation-specific security, change, safety, privacy, regulatory or vendor procedures.
