---
title: Troubleshoot Linux DNS resolution
slug: troubleshoot-linux-dns-resolution
description: 'Enterprise runbook to troubleshoot linux dns resolution without skipping evidence, verification, rollback or escalation requirements.'
...
tags:
  ...
  - troubleshoot
error_codes: []
tldr: 'Confirm scope and authorisation, capture evidence, isolate the failing layer, apply the least disruptive approved remediation for troubleshoot linux dns resolution, verify the original business task, and escalate with complete logs if recovery is not achieved.'
related_symptoms:
  - linux-dns-resolution
  - linux-or-developer-tooling-is-not-working
symptom_weights:
  linux-dns-resolution: 10
...
permalink: /procedures/troubleshoot-linux-dns-resolution/
layout: article
...
---
## Purpose and scope
Use this runbook for **troubleshoot linux dns resolution** in a managed enterprise environment. It covers intake, evidence, safe diagnosis, remediation, verification, documentation and escalation. It does not replace organisation-specific security, change, safety, privacy, regulatory or vendor procedures.

## Mandatory Batch B-2 controls
These audit-derived controls are mandatory before more invasive remediation.

### Pre-checks
1. Check the active resolver configuration (`resolvectl`, NetworkManager or `/etc/resolv.conf` as appropriate) rather than assuming one static file owns DNS.
2. Test `dig`/`nslookup` against the configured DNS server and an explicit known-good server where policy permits.
3. Check system time/NTP and DNSSEC-related errors where applicable.

### Rollback / undo
- Back up or record the resolver configuration before change and restore the previous approved resolver settings if name resolution worsens.
