---
title: Troubleshoot a Windows Update failure
slug: troubleshoot-a-windows-update-failure
description: 'Enterprise runbook to troubleshoot a windows update failure without skipping evidence, verification, rollback or escalation requirements.'
...
tags:
  ...
  - windows-endpoints
error_codes: []
tldr: 'Confirm scope and authorisation, capture evidence, isolate the failing layer, apply the least disruptive approved remediation for troubleshoot a windows update failure, verify the original business task, and escalate with complete logs if recovery is not achieved.'
related_symptoms:
  - a-windows-update-failure
  - windows-device-is-not-working-correctly
  - a-windows-error-or-performance-issue-is-reported
symptom_weights:
  a-windows-update-failure: 10
...
permalink: /procedures/troubleshoot-a-windows-update-failure/
layout: article
...
---
## Purpose and scope
Use this runbook for **troubleshoot a windows update failure** in a managed enterprise environment. It covers intake, evidence, safe diagnosis, remediation, verification, documentation and escalation. It does not replace organisation-specific security, change, safety, privacy, regulatory or vendor procedures.

## Mandatory Batch B-4 controls
These audit-derived controls are mandatory before more invasive remediation.

### Pre-checks
1. Check free disk space and the exact Windows Update error code before clearing update components.
2. Confirm the device can reach the organisation's configured update source (Windows Update for Business/WSUS/management service).
3. Review Windows Update/Event logs and update history for the failed KB/build and recent servicing-stack/reboot state.

### Rollback / undo
- If an installed update is confirmed to cause regression, use the organisation's supported uninstall/known-issue rollback or recovery process. Preserve user data and record the KB/build; do not remove security updates without change/security approval.
...
### Targeted checks
- Record update IDs, error codes, pending restart, free space and management policy.
- Review update logs and confirm the update is approved for the device ring.

### Targeted remediation sequence
1. Clear only documented update state, repair components or retry through management.
2. Rollback a confirmed bad update only through the approved change path.

### Scenario-specific success criterion
The approved update installs, the device restarts cleanly and reports compliant.
