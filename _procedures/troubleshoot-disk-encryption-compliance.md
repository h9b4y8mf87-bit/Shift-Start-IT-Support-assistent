---
title: Troubleshoot disk-encryption compliance
slug: troubleshoot-disk-encryption-compliance
description: Enterprise runbook to troubleshoot disk-encryption compliance without skipping evidence, verification, rollback or escalation requirements.
...
- troubleshoot
error_codes: []
tldr: Confirm scope and authorisation, capture evidence, isolate the failing layer, apply the least disruptive approved remediation for troubleshoot disk-encryption compliance, verify the original business task, and escalate with complete logs if recovery is not achieved.
related_symptoms:
- disk-encryption-compliance
- a-security-or-compliance-concern-is-reported
symptom_weights:
  disk-encryption-compliance: 10
...
permalink: /procedures/troubleshoot-disk-encryption-compliance/
layout: article
...
### Targeted checks
- Confirm the exact condition described by “Troubleshoot disk-encryption compliance” and identify its first failing dependency.
