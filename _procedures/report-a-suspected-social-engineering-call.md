...
description: 'Enterprise runbook to report a suspected social-engineering call without skipping evidence, verification, rollback or escalation requirements.'
...
tags:
  ...
  - suspected
error_codes: []
tldr: 'Confirm scope and authorisation, capture evidence, isolate the failing layer, apply the least disruptive approved remediation for report a suspected social-engineering call, verify the original business task, and escalate with complete logs if recovery is not achieved.'
related_symptoms:
...
---
## Purpose and scope
Use this runbook for **report a suspected social-engineering call** in a managed enterprise environment. It covers intake, evidence, safe diagnosis, remediation, verification, documentation and escalation. It does not replace organisation-specific security, change, safety, privacy, regulatory or vendor procedures.
...
### Targeted checks
- Confirm the exact condition described by “Report a suspected social-engineering call” and identify its first failing dependency.
...
The exact scenario “Report a suspected social-engineering call” is resolved and the original business task succeeds.
