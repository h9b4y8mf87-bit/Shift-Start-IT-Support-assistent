---
title: Provision a device with Windows Autopilot
slug: provision-a-device-with-windows-autopilot
description: 'Enterprise runbook to provision a device with windows autopilot without skipping evidence, verification, rollback or escalation requirements.'
...
tags:
  ...
  - with
error_codes: []
tldr: 'Confirm scope and authorisation, capture evidence, isolate the failing layer, apply the least disruptive approved remediation for provision a device with windows autopilot, verify the original business task, and escalate with complete logs if recovery is not achieved.'
related_symptoms:
  - need-to-provision-a-device-with-windows-autopilot
  - windows-device-is-not-working-correctly
  - a-windows-error-or-performance-issue-is-reported
symptom_weights:
  need-to-provision-a-device-with-windows-autopilot: 10
...
permalink: /procedures/provision-a-device-with-windows-autopilot/
layout: article
...
---
## Purpose and scope
Use this runbook for **provision a device with windows autopilot** in a managed enterprise environment. It covers intake, evidence, safe diagnosis, remediation, verification, documentation and escalation. It does not replace organisation-specific security, change, safety, privacy, regulatory or vendor procedures.

## Mandatory Batch B-3 controls
These audit-derived controls are mandatory before more invasive remediation.

### Pre-checks
1. Verify the device is registered in Windows Autopilot/Intune with the expected hardware identity and ownership.
2. Confirm the correct deployment profile, user assignment, licensing and required apps/policies.
3. Confirm supported network/internet access and no unresolved hardware/TPM issue before reprovisioning.

### Rollback / undo
- If provisioning fails, preserve user/business data and use the organisation's supported Intune/Autopilot reset or reprovisioning workflow. Do not use arbitrary sysprep/reset actions without authorisation and a data-protection plan.
...
### Targeted checks
- Validate the approved identity source, start date, manager, department, location and role profile.
- Compare the requested access to the approved role-based access package.

### Targeted remediation sequence
1. Create or synchronise the identity, assign baseline groups/licences and provision required devices.
2. Have the manager validate access without sharing credentials.
