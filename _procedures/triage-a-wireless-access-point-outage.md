...
symptom_weights:
  a-wireless-access-point-outage: 10
...
### Pre-checks
1. Check wireless controller/cloud registration state before rebooting the AP.
2. Check switch-port link/VLAN/trunk and PoE power budget before restart/replacement.
3. Check neighbouring APs to determine whether the outage is local or wider.

### Rollback / undo
- Capture AP/controller and switch-port configuration first. For remote Cisco IOS/IOS-XE switch changes, schedule reload in 10 and cancel only after verified recovery; use the vendor-equivalent rollback safety mechanism elsewhere.
...
The exact scenario “Triage a wireless access point outage” is resolved and the original business task succeeds.
