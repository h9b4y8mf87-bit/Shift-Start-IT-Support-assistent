...
description: Enterprise runbook to troubleshoot a browser extension conflict without skipping evidence, verification, rollback or escalation requirements.
...
- troubleshoot
error_codes: []
tldr: Confirm scope and authorisation, capture evidence, isolate the failing layer, apply the least disruptive approved remediation for troubleshoot a browser extension conflict, verify the original business task, and escalate with complete logs if recovery is not achieved.
related_symptoms:
