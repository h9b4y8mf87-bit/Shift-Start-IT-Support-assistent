---
title: Clear and rebuild the Teams cache
slug: clear-and-rebuild-the-teams-cache
description: 'Enterprise runbook to clear and rebuild the teams cache without skipping evidence, verification, rollback or escalation requirements.'
...
tags:
  ...
  - the
error_codes: []
tldr: 'Confirm scope and authorisation, capture evidence, isolate the failing layer, apply the least disruptive approved remediation for clear and rebuild the teams cache, verify the original business task, and escalate with complete logs if recovery is not achieved.'
related_symptoms:
  - and-rebuild-the-teams-cache
  - microsoft-365-feature-is-not-working
  - cloud-collaboration-access-is-failing
symptom_weights:
  and-rebuild-the-teams-cache: 10
...
permalink: /procedures/clear-and-rebuild-the-teams-cache/
layout: article
...
---
## Purpose and scope
Use this runbook for **clear and rebuild the teams cache** in a managed enterprise environment. It covers intake, evidence, safe diagnosis, remediation, verification, documentation and escalation. It does not replace organisation-specific security, change, safety, privacy, regulatory or vendor procedures.

## Mandatory Batch B-1 controls
These audit-derived controls are mandatory before more invasive remediation.

### Pre-checks
1. Check Microsoft 365/Teams service health.
2. Test Teams on the web to prove the issue is local to the desktop client.
3. Confirm which Teams client generation is installed before using a cache path.

### Rollback / undo
- Close Teams and preserve/rename relevant client cache/profile data before deletion where supported; restore preserved state or reinstall through the approved channel if rebuild fails.
