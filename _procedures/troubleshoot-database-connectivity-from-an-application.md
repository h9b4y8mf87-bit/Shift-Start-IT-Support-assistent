...
description: Enterprise runbook to troubleshoot database connectivity from an application without skipping evidence,
...
approval_required: Follow organisational policy for privileged, destructive, security-sensitive or service-impacting
  actions.
permalink: /procedures/troubleshoot-database-connectivity-from-an-application/
layout: article
---
## Purpose and scope
...
### Targeted checks
- Confirm the exact condition described by “Troubleshoot database connectivity from an application” and identify its first failing dependency.
...
The exact scenario “Troubleshoot database connectivity from an application” is resolved and the original business task succeeds.
