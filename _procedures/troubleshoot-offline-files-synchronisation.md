---
title: Troubleshoot Offline Files synchronisation
slug: troubleshoot-offline-files-synchronisation
description: 'Enterprise runbook to troubleshoot offline files synchronisation without skipping evidence, verification, rollback or escalation requirements.'
...
tags:
  ...
  - windows-endpoints
error_codes: []
tldr: 'Confirm scope and authorisation, capture evidence, isolate the failing layer, apply the least disruptive approved remediation for troubleshoot offline files synchronisation, verify the original business task, and escalate with complete logs if recovery is not achieved.'
related_symptoms:
  - offline-files-synchronisation
  - windows-device-is-not-working-correctly
  - a-windows-error-or-performance-issue-is-reported
symptom_weights:
  offline-files-synchronisation: 10
...
permalink: /procedures/troubleshoot-offline-files-synchronisation/
layout: article
...
---
## Purpose and scope
Use this runbook for **troubleshoot offline files synchronisation** in a managed enterprise environment. It covers intake, evidence, safe diagnosis, remediation, verification, documentation and escalation. It does not replace organisation-specific security, change, safety, privacy, regulatory or vendor procedures.

## Mandatory Batch B-6 controls
These audit-derived controls are mandatory before more invasive remediation.

### Pre-checks
1. Confirm network/VPN reachability to the authoritative file server/share and verify the server copy is accessible.
2. Check Windows Offline Files/Sync Center state, cache capacity and exact conflict/error rather than deleting the CSC cache.
3. Identify file locks/conflicts and establish which copy is authoritative before reconciling data.

### Rollback / undo
- Preserve unsynchronised local data before reset. If an authorised GPO/configuration change caused the issue, restore the previous policy. Resolve conflicts from a verified authoritative copy or backup; do not overwrite local/server data blindly.
