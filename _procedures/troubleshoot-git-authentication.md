---
title: Troubleshoot Git authentication
slug: troubleshoot-git-authentication
description: 'Enterprise runbook to troubleshoot git authentication without skipping evidence, verification, rollback or escalation requirements.'
...
tags:
  ...
  - troubleshoot
error_codes: []
tldr: 'Confirm scope and authorisation, capture evidence, isolate the failing layer, apply the least disruptive approved remediation for troubleshoot git authentication, verify the original business task, and escalate with complete logs if recovery is not achieved.'
related_symptoms:
  - git-authentication
  - linux-or-developer-tooling-is-not-working
symptom_weights:
  git-authentication: 10
...
permalink: /procedures/troubleshoot-git-authentication/
layout: article
...
---
## Purpose and scope
Use this runbook for **troubleshoot git authentication** in a managed enterprise environment. It covers intake, evidence, safe diagnosis, remediation, verification, documentation and escalation. It does not replace organisation-specific security, change, safety, privacy, regulatory or vendor procedures.

## Mandatory Batch B-2 controls
These audit-derived controls are mandatory before more invasive remediation.

### Pre-checks
1. Determine whether authentication uses SSH, HTTPS/PAT, SSO or an organisation-managed credential helper.
2. Check token/key expiry or revocation and repository/group entitlement without exposing secret material.
3. Test the web interface or another approved authentication path to distinguish account entitlement from local Git configuration.

### Rollback / undo
- Do not restore revoked or compromised credentials. Record the previous approved credential method/configuration and revert only the client configuration if a new approved credential method causes regression.
...
### Targeted checks
- Capture command output, exit code, version, environment, proxy, permissions, logs and uncommitted work.
- Compare a clean shell/profile and supported runtime or container.

### Targeted remediation sequence
1. Repair the specific configuration, credential, package or service without deleting repositories or work.
2. Rebuild only the isolated environment after exporting configuration and data.
