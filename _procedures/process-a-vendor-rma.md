...
description: Enterprise runbook to process a vendor rma without skipping evidence, verification, rollback or escalation
  requirements.
...
tldr: Confirm scope and authorisation, capture evidence, isolate the failing layer, apply the least disruptive approved
  remediation for process a vendor rma, verify the original business task, and escalate with complete logs if recovery
  is not achieved.
related_symptoms:
