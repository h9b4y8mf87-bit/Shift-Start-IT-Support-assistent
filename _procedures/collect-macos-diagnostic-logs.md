...
description: Enterprise runbook to collect macos diagnostic logs without skipping evidence, verification, rollback
...
tldr: Confirm scope and authorisation, capture evidence, isolate the failing layer, apply the least disruptive approved
  remediation for collect macos diagnostic logs, verify the original business task, and escalate with complete logs
...
approval_required: Follow organisational policy for privileged, destructive, security-sensitive or service-impacting
  actions.
permalink: /procedures/collect-macos-diagnostic-logs/
layout: article
---
## Purpose and scope
...
### Targeted checks
- Confirm the exact condition described by “Collect macOS diagnostic logs” and identify its first failing dependency.
...
The exact scenario “Collect macOS diagnostic logs” is resolved and the original business task succeeds.
