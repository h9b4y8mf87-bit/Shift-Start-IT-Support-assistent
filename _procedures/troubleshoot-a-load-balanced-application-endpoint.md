---
title: Troubleshoot a load-balanced application endpoint
slug: troubleshoot-a-load-balanced-application-endpoint
description: Enterprise runbook to troubleshoot a load-balanced application endpoint without skipping evidence, verification, rollback or escalation requirements.
...
- troubleshoot
error_codes: []
tldr: Confirm scope and authorisation, capture evidence, isolate the failing layer, apply the least disruptive approved remediation for troubleshoot a load-balanced application endpoint, verify the original business task, and escalate with complete logs if recovery is not achieved.
related_symptoms:
- a-load-balanced-application-endpoint
- a-cloud-or-virtual-resource-is-degraded
symptom_weights:
  a-load-balanced-application-endpoint: 10
...
permalink: /procedures/troubleshoot-a-load-balanced-application-endpoint/
layout: article
...
### Targeted checks
- Confirm the exact condition described by “Troubleshoot a load-balanced application endpoint” and identify its first failing dependency.
...
The exact scenario “Troubleshoot a load-balanced application endpoint” is resolved and the original business task succeeds.
