---
title: Install or renew a mobile certificate
slug: install-or-renew-a-mobile-certificate
description: 'Enterprise runbook to install or renew a mobile certificate without skipping evidence, verification, rollback or escalation requirements.'
...
tags:
  ...
  - renew
error_codes: []
tldr: 'Confirm scope and authorisation, capture evidence, isolate the failing layer, apply the least disruptive approved remediation for install or renew a mobile certificate, verify the original business task, and escalate with complete logs if recovery is not achieved.'
related_symptoms:
  - need-to-install-or-renew-a-mobile-certificate
  - corporate-access-on-a-mobile-device-is-not-working
  - mobile-device-is-not-compliant
symptom_weights:
  need-to-install-or-renew-a-mobile-certificate: 10
...
permalink: /procedures/install-or-renew-a-mobile-certificate/
layout: article
...
---
## Purpose and scope
Use this runbook for **install or renew a mobile certificate** in a managed enterprise environment. It covers intake, evidence, safe diagnosis, remediation, verification, documentation and escalation. It does not replace organisation-specific security, change, safety, privacy, regulatory or vendor procedures.

## Mandatory Batch B-3 controls
These audit-derived controls are mandatory before more invasive remediation.

### Pre-checks
1. Verify device ownership/enrolment/compliance and the user's identity before certificate issuance.
2. Check CA/NDES/SCEP or equivalent certificate service health and the intended certificate template/profile.
3. Confirm current certificate subject, expiry, EKU and trust chain without exporting private keys.

### Rollback / undo
- Do not export or restore private keys as a rollback shortcut. If the new certificate/profile causes regression, revoke the incorrect certificate where authorised and redeploy the previous approved profile or issue a fresh valid certificate through the CA/MDM workflow.
...
### Targeted checks
- Confirm the exact condition described by “Install or renew a mobile certificate” and identify its first failing dependency.
...
The exact scenario “Install or renew a mobile certificate” is resolved and the original business task succeeds.
