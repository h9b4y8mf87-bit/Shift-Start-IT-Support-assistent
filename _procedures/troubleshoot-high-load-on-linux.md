...
description: Enterprise runbook to troubleshoot high load on linux without skipping evidence, verification, rollback
...
tldr: Confirm scope and authorisation, capture evidence, isolate the failing layer, apply the least disruptive approved
  remediation for troubleshoot high load on linux, verify the original business task, and escalate with complete
