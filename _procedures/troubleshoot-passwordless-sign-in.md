---
title: Troubleshoot passwordless sign-in
slug: troubleshoot-passwordless-sign-in
description: 'Enterprise runbook to troubleshoot passwordless sign-in without skipping evidence, verification, rollback or escalation requirements.'
...
tags:
  ...
  - troubleshoot
error_codes: []
tldr: 'Confirm scope and authorisation, capture evidence, isolate the failing layer, apply the least disruptive approved remediation for troubleshoot passwordless sign-in, verify the original business task, and escalate with complete logs if recovery is not achieved.'
related_symptoms:
  - passwordless-sign-in
  - cannot-sign-in
  - access-is-denied-or-missing
symptom_weights:
  passwordless-sign-in: 10
  cannot-sign-in: 3
...
permalink: /procedures/troubleshoot-passwordless-sign-in/
layout: article
...
---
## Purpose and scope
Use this runbook for **troubleshoot passwordless sign-in** in a managed enterprise environment. It covers intake, evidence, safe diagnosis, remediation, verification, documentation and escalation. It does not replace organisation-specific security, change, safety, privacy, regulatory or vendor procedures.
...
### Targeted checks
- Review registered authentication methods, sign-in logs, device time and policy requirements.
- Confirm a recovery method exists before deleting or replacing an authentication method.

### Targeted remediation sequence
1. Use the approved proofing process, register the replacement method and revoke the lost method.
2. Test primary and recovery authentication from a trusted device.
...
4. **Inventory the user’s passwordless methods and recent sign-ins.** Confirm method type, device association and authentication error before resetting any FIDO2, Authenticator or Windows Hello registration.
...
Get-MgUserAuthenticationFido2Method -UserId "user@contoso.com" |
  Select-Object Id,DisplayName,Model,CreatedDateTime

Get-MgUserAuthenticationWindowsHelloForBusinessMethod -UserId "user@contoso.com" |
  Select-Object Id,DisplayName,CreatedDateTime,KeyStrength

Get-MgAuditLogSignIn -Filter "userPrincipalName eq 'user@contoso.com'" -Top 20 |
  Select-Object CreatedDateTime,AuthenticationRequirement,AuthenticationDetails,Status
{% endcapture %}
{% include command.html shell="powershell" label="Passwordless sign-in evidence" command=enterprise_command %}

   <div class="expected"><strong>Expected result:</strong> Registered passwordless methods and the failing authentication stage are identified before a method is removed or re-enrolled.</div>
