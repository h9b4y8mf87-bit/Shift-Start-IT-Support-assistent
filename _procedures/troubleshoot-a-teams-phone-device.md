---
title: Troubleshoot a Teams Phone device
slug: troubleshoot-a-teams-phone-device
description: 'Enterprise runbook to troubleshoot a teams phone device without skipping evidence, verification, rollback or escalation requirements.'
...
error_codes: []
tldr: 'Confirm scope and authorisation, capture evidence, isolate the failing layer, apply the least disruptive approved remediation for troubleshoot a teams phone device, verify the original business task, and escalate with complete logs if recovery is not achieved.'
related_symptoms:
  - a-teams-phone-device
  - calling-audio-video-or-meeting-room-technology-is-not-working
symptom_weights:
  a-teams-phone-device: 10
...
permalink: /procedures/troubleshoot-a-teams-phone-device/
layout: article
...
---
## Purpose and scope
Use this runbook for **troubleshoot a teams phone device** in a managed enterprise environment. It covers intake, evidence, safe diagnosis, remediation, verification, documentation and escalation. It does not replace organisation-specific security, change, safety, privacy, regulatory or vendor procedures.

## Mandatory Batch B-2 controls
These audit-derived controls are mandatory before more invasive remediation.

### Pre-checks
1. Check Microsoft 365 / Teams service health before changing the physical device.
2. Check the phone's network link and power source, including PoE state where the model uses PoE.
3. Check device registration, health and assigned configuration/profile in the Teams Admin Center.

### Rollback / undo
- Record the current device configuration/update state before a change. Revert the assigned profile or firmware/update ring only where the platform supports an approved rollback; otherwise use the vendor-supported recovery path.
...
### Targeted checks
- Check service health, client/web comparison, selected devices, permissions, network quality and meeting policy.
- Capture meeting/call ID, time and affected participants.

### Targeted remediation sequence
1. Refresh the client/cache or correct device and policy assignment.
2. Escalate tenant, media, carrier or room-system faults with call diagnostics.
