...
description: 'Enterprise runbook to replace a failed corporate device without skipping evidence, verification, rollback or escalation requirements.'
...
tags:
  ...
  - replace
error_codes: []
tldr: 'Confirm scope and authorisation, capture evidence, isolate the failing layer, apply the least disruptive approved remediation for replace a failed corporate device, verify the original business task, and escalate with complete logs if recovery is not achieved.'
related_symptoms:
  - replace-a-failed-corporate-device
  - hardware-or-peripheral-is-not-functioning
symptom_weights:
  replace-a-failed-corporate-device: 10
...
permalink: /procedures/replace-a-failed-corporate-device/
layout: article
...
### Targeted checks
- Confirm the exact condition described by “Replace a failed corporate device” and identify its first failing dependency.
...
The exact scenario “Replace a failed corporate device” is resolved and the original business task succeeds.
