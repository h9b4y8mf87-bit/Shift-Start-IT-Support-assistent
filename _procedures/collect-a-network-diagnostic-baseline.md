---
title: Collect a network diagnostic baseline
slug: collect-a-network-diagnostic-baseline
description: Enterprise runbook to collect a network diagnostic baseline without skipping evidence, verification, rollback or escalation requirements.
...
error_codes: []
tldr: Confirm scope and authorisation, capture evidence, isolate the failing layer, apply the least disruptive approved remediation for collect a network diagnostic baseline, verify the original business task, and escalate with complete logs if recovery is not achieved.
related_symptoms:
- need-to-collect-a-network-diagnostic-baseline
- network-or-internet-access-is-unavailable
- connection-is-slow-or-intermittent
symptom_weights:
  need-to-collect-a-network-diagnostic-baseline: 10
...
permalink: /procedures/collect-a-network-diagnostic-baseline/
layout: article
...
---
## Purpose and scope
Use this runbook for **collect a network diagnostic baseline** in a managed enterprise environment. It covers intake, evidence, safe diagnosis, remediation, verification, documentation and escalation. It does not replace organisation-specific security, change, safety, privacy, regulatory or vendor procedures.
...
### Targeted checks
- Confirm the exact condition described by “Collect a network diagnostic baseline” and identify its first failing dependency.
...
The exact scenario “Collect a network diagnostic baseline” is resolved and the original business task succeeds.
