---
title: Troubleshoot VDI performance
slug: troubleshoot-vdi-performance
description: 'Enterprise runbook to troubleshoot vdi performance without skipping evidence, verification, rollback or escalation requirements.'
...
tags:
  ...
  - vdi
error_codes: []
tldr: 'Confirm scope and authorisation, capture evidence, isolate the failing layer, apply the least disruptive approved remediation for troubleshoot vdi performance, verify the original business task, and escalate with complete logs if recovery is not achieved.'
related_symptoms:
  - vdi-performance
...
permalink: /procedures/troubleshoot-vdi-performance/
layout: article
...
---
## Purpose and scope
Use this runbook for **troubleshoot vdi performance** in a managed enterprise environment. It covers intake, evidence, safe diagnosis, remediation, verification, documentation and escalation. It does not replace organisation-specific security, change, safety, privacy, regulatory or vendor procedures.

## Mandatory Batch B-6 controls
These audit-derived controls are mandatory before more invasive remediation.

### Pre-checks
1. Determine whether performance affects one session, one host, one pool/site or all users.
2. Check host CPU, memory, storage I/O, session density and broker/VDA/agent health.
3. Check client-to-gateway/host latency, packet loss and bandwidth plus recent image/policy/host changes.

### Rollback / undo
- Capture host/image/policy state before change. If a confirmed patch/image/configuration change causes degradation, restore the previous approved image/policy or drain/migrate sessions to a known-good host under change control.
