---
title: Troubleshoot high CPU utilisation
slug: troubleshoot-high-cpu-utilisation
description: 'Enterprise runbook to troubleshoot high cpu utilisation without skipping evidence, verification, rollback or escalation requirements.'
...
tags:
  ...
  - windows-endpoints
error_codes: []
tldr: 'Confirm scope and authorisation, capture evidence, isolate the failing layer, apply the least disruptive approved remediation for troubleshoot high cpu utilisation, verify the original business task, and escalate with complete logs if recovery is not achieved.'
related_symptoms:
  - high-cpu-utilisation
  - windows-device-is-not-working-correctly
  - a-windows-error-or-performance-issue-is-reported
symptom_weights:
  high-cpu-utilisation: 10
...
permalink: /procedures/troubleshoot-high-cpu-utilisation/
layout: article
...
canonical_role: definitive-high-cpu-utilisation-runbook
---
## Purpose and scope
Use this runbook for **troubleshoot high cpu utilisation** in a managed enterprise environment. It covers intake, evidence, safe diagnosis, remediation, verification, documentation and escalation. It does not replace organisation-specific security, change, safety, privacy, regulatory or vendor procedures.

## Mandatory Batch B-5 controls
These audit-derived controls are mandatory before more invasive remediation.

### Pre-checks
1. Determine whether the high CPU is a single endpoint or service/server-wide condition and identify the top processes with Task Manager/Performance Monitor or approved tooling.
2. Correlate process/service errors, recent deployments/updates and scheduled activity with the high-CPU window.
3. Check whether the process is legitimate expected load, a runaway workload or potentially malicious before terminating it.

### Rollback / undo
- Capture process/service and recent-change state. If a confirmed deployment/update caused the load, restore the previous approved version/configuration. Restart or stop a service only under change authority and after checking dependencies.
...
### Targeted checks
- Capture utilisation over time and identify the process, session or workload responsible.
- Correlate with recent updates, scheduled jobs, malware alerts, capacity and user actions.

### Targeted remediation sequence
1. Restart only the affected process when safe, correct its configuration or remove the confirmed trigger.
2. Escalate capacity or code defects with performance evidence rather than repeatedly rebooting.
...
4. **Run non-destructive diagnostics.** Review device management status, Event Viewer, Reliability Monitor, recent updates or changes, resource usage, storage health and device-driver state.

{% capture enterprise_command %}
Get-ComputerInfo | Select CsName,WindowsProductName,WindowsVersion,OsBuildNumber; Get-CimInstance Win32_OperatingSystem | Select LastBootUpTime,FreePhysicalMemory
{% endcapture %}
{% include command.html shell="powershell" label="Windows baseline" command=enterprise_command %}
...
5. **Apply the primary approved remediation.** Apply the least disruptive supported remediation first: restart the affected component, repair configuration, update policy or use the approved troubleshooter.
...
7. **Apply secondary remediation only when justified.** If the issue persists, repair or reinstall the affected component, rollback the confirmed bad change, or use the approved device recovery workflow.
...
9. **Complete end-to-end verification.** The original user task succeeds after a restart or fresh sign-in, device health remains stable and management/compliance reporting is normal.
