---
title: Perform a device health check
slug: perform-a-device-health-check
description: Enterprise runbook to perform a device health check without skipping evidence, verification, rollback or escalation requirements.
...
- perform
error_codes: []
tldr: Confirm scope and authorisation, capture evidence, isolate the failing layer, apply the least disruptive approved remediation for perform a device health check, verify the original business task, and escalate with complete logs if recovery is not achieved.
related_symptoms:
- need-to-a-device-health-check
- a-device-software-or-asset-lifecycle-request-is-required
symptom_weights:
  need-to-a-device-health-check: 10
...
permalink: /procedures/perform-a-device-health-check/
layout: article
...
---
## Purpose and scope
Use this runbook for **perform a device health check** in a managed enterprise environment. It covers intake, evidence, safe diagnosis, remediation, verification, documentation and escalation. It does not replace organisation-specific security, change, safety, privacy, regulatory or vendor procedures.
...
### Targeted checks
- Confirm the exact condition described by “Perform a device health check” and identify its first failing dependency.
...
The exact scenario “Perform a device health check” is resolved and the original business task succeeds.
