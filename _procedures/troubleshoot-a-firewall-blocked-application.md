---
title: Troubleshoot a firewall-blocked application
slug: troubleshoot-a-firewall-blocked-application
description: 'Enterprise runbook to troubleshoot a firewall-blocked application without skipping evidence, verification, rollback or escalation requirements.'
...
tags:
  ...
  - troubleshoot
error_codes: []
tldr: 'Confirm scope and authorisation, capture evidence, isolate the failing layer, apply the least disruptive approved remediation for troubleshoot a firewall-blocked application, verify the original business task, and escalate with complete logs if recovery is not achieved.'
related_symptoms:
  - a-firewall-blocked-application
  - network-or-internet-access-is-unavailable
  - connection-is-slow-or-intermittent
symptom_weights:
  a-firewall-blocked-application: 10
...
permalink: /procedures/troubleshoot-a-firewall-blocked-application/
layout: article
...
---
## Purpose and scope
Use this runbook for **troubleshoot a firewall-blocked application** in a managed enterprise environment. It covers intake, evidence, safe diagnosis, remediation, verification, documentation and escalation. It does not replace organisation-specific security, change, safety, privacy, regulatory or vendor procedures.

## Mandatory Batch B-1 controls
These audit-derived controls are mandatory before more invasive remediation.

### Pre-checks
1. Check firewall logs for the exact source, destination, protocol and port denial.
2. Verify the application's documented network requirements before adding a rule.
3. Check NAT/PAT and upstream security controls.

### Rollback / undo
- Capture the current ACL/rule state; if the new rule causes unintended access, remove/revert only that change and restore the prior known-good policy.
...
### Targeted checks
- Capture the blocked URL, destination, port, policy message, user/device identity and time.
- Check approved proxy/PAC, certificate, posture and authentication status.

### Targeted remediation sequence
1. Correct the endpoint profile or submit a justified policy request to the control owner.
2. Never bypass filtering, firewall or NAC with an unapproved alternate path.

### Scenario-specific success criterion
The approved destination works and the security control remains enforced.
