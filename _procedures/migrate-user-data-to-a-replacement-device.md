...
description: 'Enterprise runbook to migrate user data to a replacement device without skipping evidence, verification, rollback or escalation requirements.'
...
tags:
  ...
  - user
error_codes: []
tldr: 'Confirm scope and authorisation, capture evidence, isolate the failing layer, apply the least disruptive approved remediation for migrate user data to a replacement device, verify the original business task, and escalate with complete logs if recovery is not achieved.'
related_symptoms:
  - migrate-user-data-to-a-replacement-device
  - business-data-is-missing-corrupted-or-requires-recovery
symptom_weights:
  migrate-user-data-to-a-replacement-device: 10
...
permalink: /procedures/migrate-user-data-to-a-replacement-device/
layout: article
...
---
## Purpose and scope
Use this runbook for **migrate user data to a replacement device** in a managed enterprise environment. It covers intake, evidence, safe diagnosis, remediation, verification, documentation and escalation. It does not replace organisation-specific security, change, safety, privacy, regulatory or vendor procedures.
...
### Targeted checks
- Confirm the exact condition described by “Migrate user data to a replacement device” and identify its first failing dependency.
...
The exact scenario “Migrate user data to a replacement device” is resolved and the original business task succeeds.
