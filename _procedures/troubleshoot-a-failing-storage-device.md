---
title: Troubleshoot a failing storage device
slug: troubleshoot-a-failing-storage-device
description: 'Enterprise runbook to troubleshoot a failing storage device without skipping evidence, verification, rollback or escalation requirements.'
...
tags:
  ...
  - troubleshoot
error_codes: []
tldr: 'Confirm scope and authorisation, capture evidence, isolate the failing layer, apply the least disruptive approved remediation for troubleshoot a failing storage device, verify the original business task, and escalate with complete logs if recovery is not achieved.'
related_symptoms:
  - a-failing-storage-device
  - hardware-or-peripheral-is-not-functioning
symptom_weights:
  a-failing-storage-device: 10
...
permalink: /procedures/troubleshoot-a-failing-storage-device/
layout: article
...
---
## Purpose and scope
Use this runbook for **troubleshoot a failing storage device** in a managed enterprise environment. It covers intake, evidence, safe diagnosis, remediation, verification, documentation and escalation. It does not replace organisation-specific security, change, safety, privacy, regulatory or vendor procedures.
...
### Targeted checks
- Confirm the exact condition described by “Troubleshoot a failing storage device” and identify its first failing dependency.
...
The exact scenario “Troubleshoot a failing storage device” is resolved and the original business task succeeds.
