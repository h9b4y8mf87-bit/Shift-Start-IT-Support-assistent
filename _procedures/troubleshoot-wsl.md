...
description: Enterprise runbook to troubleshoot wsl without skipping evidence, verification, rollback or escalation
  requirements.
...
tldr: Confirm scope and authorisation, capture evidence, isolate the failing layer, apply the least disruptive approved
  remediation for troubleshoot wsl, verify the original business task, and escalate with complete logs if recovery
  is not achieved.
related_symptoms:
- wsl
